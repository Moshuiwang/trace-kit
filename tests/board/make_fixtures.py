...
# ---------------------------------------------------------------- 结果构造
def commit(s, at, subject, author="agent-a", refs=""):
    return {"sha": s, "at": at, "authored": at, "author": author, "subject": subject, "refs": refs}


def pr(number, title, created, merged="", author="agent-a", merged_by="", head="", merge_commit="",
       body="", reviews=(), checks=(), state="OPEN", draft=False, closed=""):
    return {
        ...
        "headRefName": head, "baseRefName": "main", "mergeCommit": merge_commit,
...
def complex_table(*, contract_section: bool, contract_ptr: bool, a1: bool, a2: bool, k1: bool, r1: bool) -> str:
    ...
    wave2 = ["- [%s] K-1 候选冻结（`%s`） [t:review est:10m needs:S-A1,S-A2]" % (box(k1), CAND[:12]),
...
def base_rows(*, log, hist, prs, iss, runs, tags=(), gh_tags=(), branches=(), worktrees=(), contract=None,
              release_runs=None, quotes=()):
    rows = [
        ok_("git.log", list(log)),
...
def build_simple(moment):
    ...
    if moment == 0:
        rows = base_rows(log=S_COMMITS[:1], hist=history(hist_rows, S_PATH, SIMPLE_IDS), prs=[S_CONTRACT_PR],
                         iss=issue(S_NO, T("02:00"), comments=comments), runs=[], branches=S_BRANCHES, contract=S_CONTRACT)
        ...
        rows = base_rows(log=S_COMMITS, hist=history(hist_rows, S_PATH, SIMPLE_IDS), prs=[S_CONTRACT_PR, S_PR11],
                         ...
                         worktrees=[S_WT], contract=S_CONTRACT)
...
X_BATCH_PR = pr(29, "feat(#8): 批次分支", T("02:00"), head=X_BRANCH, body="Trace #8 批次 PR。")
...
def build_complex(fault):
    ...
    rows = base_rows(log=log, hist=history(hist, X_PATH, X_IDS), prs=prs, iss=issue(X_NO, T("01:00"), comments=comments),
                     runs=runs, branches=branches, worktrees=worktrees, contract=X_CONTRACT)
...
SUMMARY = {
    ...
    "complex-f3": "复杂链故障 3：审核出结论后分支 HEAD 从候选 `c3c3…` 变成 `d4d4…` → 候选冻结 K-1 与审核结论 R-1 双双失效灰暗、卡片带 ↺重审来源；模块轮数 审 1 实 ＋ 外 1 推（时间窗回落）。",
    ...
    "unknown-cmd": "证据源配置声明的 `config.production` 命令超时：该级阶段显示「未知」并进头部阻塞，其余四级照常判定（不沿用旧结论）。",
...
CHECKS: dict[str, list] = {
    "trace1-replay": [
        ("最大空档 299 分钟", "最大空档 299推 分钟", "simple", OK),
        ("空档归因暂停 219 分钟", "commit 2183910 09-02 10:48:20 → PR #5 09-02 15:46:55；归因暂停 219报 分钟", "why", OK + "（头部存疑行 150 列截断，只在 --why 完整可见）"),
    ...
    "complex-f3": [
        ...
        ("候选卡片灰暗", "K-1 审核 · 失效", "complex", OK + "；候选冻结必须标 t:review 才会失效，见报告第 4 节"),
    ...
    "unknown-cmd": [
        ("该级阶段未知", "五级阶段  合入主干 是 · 已发布 是 · 预发已升级 是 · 已上生产 未知 · 收口 否", "simple", OK),
        ("阶段未知进阻塞", "阻塞      阶段未知：已上生产", "simple", OK),
        ("超时原因写进证据链", "阶段·已上生产 | 未知 | image_tag | config.production | 超时 20s", "why", OK),
