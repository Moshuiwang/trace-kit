...
- 头部固定附注「窗口状态未知，需守望者核」——除非证据源配置声明了窗口模式且取到值。**看板不判活**。
...
- **不判活**：窗口是否还活着、代理是否失联，是守望者的事（`/trace-kit:guardian`）；看板头部固定提示「需守望者核」。
...
- **配置命令不做沙箱**（账本 R1-26）：`board.toml` 里的命令是项目仓库经 PR 审阅的声明，引擎只以一次性临时目录为 cwd、独立进程组、显式超时执行；不引入第三方沙箱依赖。
- **宽字符只按东亚宽度计**（账本 R2-10）：做 NFC 规范化、零宽码点附着前一格；emoji ZWJ 序列 / 肤色修饰符 / 旗帜等字素簇不保证对齐。
- **简易版「一屏不滚动」只对链式 / 稀疏依赖保证**（账本 R2-12）：稠密 DAG（多模块互相交叉依赖）允许滚动，头部会提示「简易版 N 行超一屏」。
- **无规模预算**（账本 R2-14）：布局在主线程同步完成，按 Trace 任务表 ≤ 百级步骤设计；数千步骤级任务图不保证响应。
- 键盘：转义序列超过 64 字节仍无终结整段丢弃（账本 R2-9）；pane 小于 20×12 只显示单行「窗口过小」（账本 R2-13）。
