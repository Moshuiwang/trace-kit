...
- **事件型**（管「死没死」）：常驻轮询，只用本地证据，事件只留三类——窗口死亡、窗口输出静止 ≥ 15 分钟、查询失败；**不含「新提交」事件**（提交进度归定时型看，省 token 且实测无漏报）；不依赖网络；查询失败须显式重试并声明，不得把「查不到」读成「没事」。
...
- 额度数字优先从本机可读的数据源取（如状态条的数据文件），不向产品负责人索取；停派线按合同 §5。
