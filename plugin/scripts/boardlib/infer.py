...
- 轮数（R1-3 / A-6）：评论首行剥 `#` `>` `*` 空白后**锚定开头**匹配；「修」只计有结果的修复包评论；
...
# 轮数正则（https://github.com/Moshuiwang/lingxi/issues/578 已定；只看评论首行，剥 `#` `>` `*` 空白后锚定开头，R1-3）
...
def _classify_comment(first_line: str) -> tuple[bool, bool, bool]:
    ...
    fixpack = bool(FIXPACK_RE.search(text) and FIXPACK_RESULT_RE.search(text))
