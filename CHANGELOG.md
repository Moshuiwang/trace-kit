...
- **看板批（v0.2.0）体量**：`plugin/` 受版本控制文件总字节数 33811（v0.1.0 发布态，main `9d91a38`）→ **228740（+194929）**——其中引擎代码 177920B（91%）、`skills/board/SKILL.md` 10456B、`templates/board.toml` 3553B，其余为三句可选条款与两张表的一行。统计**不含** 8 个误随批次提交的 `plugin/scripts/boardlib/__pycache__/*.pyc`（28783B，构建产物，收口前删除并加忽略规则）。`template/` 410114 → **411319（+1205）**，只有 `docs/traces/README.md` 的看板段。
