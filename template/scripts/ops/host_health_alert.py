...
补的盲区：应用内告警跑在进程自己里，发现不了「承载它的容器本身已不健康、甚至已不在运行」——出问题的正是
发告警这条链路所在的进程。本脚本刻意跑在**容器之外**（宿主 timer/cron），只依赖 Python3 标准库与 `docker`
命令，不 import 项目包、不进任何镜像，容器全灭也跑得动；只读 `docker container inspect --format '{{json .State}}'`
（只在容器命名空间查名字、只取 State，不碰装着凭据的 Config.Env），不监听端口，只发出站 HTTP。
...
去重：同一容器同一原因只在首次进入时告警一次，原因变化算新事件；状态**只在发送成功后落盘**，发送失败不落盘、
下一轮自然重试。恢复：回到 healthy（或本来就无 healthcheck）时发一条恢复通知并清空记忆；告警态下看到 `starting`
既不算新触发也不确认恢复（否则宽限期一结束又来一条）。发送失败只写本地日志、不崩溃；`fcntl.flock` 单实例，
拿不到锁不是故障。退出码 0 = 本轮检查已完成（**不代表容器都健康**，拿不到锁也是 0）；2 = 脚本自身故障
（凭据文件缺失/权限不对、docker 不可用、状态文件写不了），需要人工介入。
凭据只从 `--env-file`（须 0600 且属主为当前用户）读 ALERT_WEBHOOK_URL，值不进 argv、不进日志；发送出口只有
`send_alert()` 一处——**这里换成你的群 webhook 格式**。
"""
...
NOT_FOUND_STDERR_MARKERS = ("No such object", "No such container")
...
def decide_action(classification: Classification, prior: ContainerState) -> tuple[str, ContainerState]:
    """去重与恢复通知的状态机；返回 (动作, 发送成功后应落盘的新状态)，本身不做 I/O。"""
    ...
    if prior.alerting and classification.reason in RECOVERY_REASONS:
        return ACTION_RECOVERY, ContainerState()
    # 告警态下遇到 starting：既不触发也不恢复，原样保留记忆，等下一轮拿到确定结果。
    return ACTION_NONE, prior if prior.alerting else ContainerState()
...
def docker_inspect_one(name: str, *, docker_bin: str, timeout_seconds: float) -> Mapping[str, object] | None:
    ...
    if proc.returncode != 0:
        if any(marker in (proc.stderr or "") for marker in NOT_FOUND_STDERR_MARKERS):
            return None
...
def load_state(path: Path) -> dict[str, ContainerState]:
    ...
    if not isinstance(raw, Mapping):
        return {}
    return {str(name): ContainerState(bool(value.get("alerting", False)), value.get("reason") or None)
            for name, value in raw.items() if isinstance(value, Mapping)}


def save_state(path: Path, states: Mapping[str, ContainerState]) -> None:
    """原子落盘（写临时文件后 os.replace），避免并发/崩溃留下半截 JSON。"""
    payload = {name: {"alerting": state.alerting, "reason": state.reason} for name, state in states.items()}
    try:
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    with single_instance_lock(Path(args.lock_file)) as acquired:
        ...
        for name in args.containers:
            ...
            action, target_state = decide_action(classification, states.get(name, ContainerState()))
