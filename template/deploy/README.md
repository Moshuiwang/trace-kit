...
出处：lingxi [#278](https://github.com/Moshuiwang/lingxi/issues/278)（epic 分支每次合入都构建镜像，五次只用一次——改为按需触发）。

批次分支上的 Story PR 只跑快检，镜像只在冻结候选时构建一次。触发方式是 git trailer：在批次分支 tip 上开一个只含空提交的临时分支，提交正文**独占一行**写 `Image-Candidate: true`（不用方括号写法，避免说明文字自指误触发），开 Story PR 合回批次分支并 squash 合并时显式指定提交信息保留该行；随后面向 `main` 的批次 PR 在 `Epic Full` 里检测到 trailer 即构建镜像并产出候选证明（检测逻辑见 `.github/workflows/ci.yml` 顶部注释）。这里换成你的：若你的仓库对批次分支有「合并前必须通过检查」的规则，空提交也走同一条 Story PR 路径。
